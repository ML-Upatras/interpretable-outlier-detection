...
def anova_test(*args):
    ...
    # Pivots
    pivots = [
        sp.mean(group) / sp.sqrt(2 * (ss_wg / df_wg) / float(n)) for group in args
    ]
...
def bonferroni_test(pivots, n):
    ...
    p_values, t_values, comparisons = map(
        list, zip(*sorted(zip(p_values, t_values, comparisons), key=lambda t: t[0]))
    )

...
def quade_test(*args):
    ...
    ranking_cases = [
        ranges_sort.index(v) + 1 + (ranges_sort.count(v) - 1) / 2.0 for v in ranges
    ]

    S = []
    W = []
    for i in range(n):
        S.append([ranking_cases[i] * (r - (k + 1) / 2.0) for r in rankings[i]])
    ...
    rankings_avg = [w / (n * (n + 1) / 2.0) for w in Wj]
    rankings_cmp = [
        r / sp.sqrt(k * (k + 1) * (2 * n + 1) * (k - 1) / (18.0 * n * (n + 1)))
...
def holm_test(ranks, control=None):
    ...
    adj_p_values = [
        min(max((k - (j + 1)) * p_values[j] for j in range(i + 1)), 1)
        for i in range(k - 1)
    ]
...
def hochberg_test(ranks, control=None):
    ...
    adj_p_values = [
        min(max((k - j) * p_values[j - 1] for j in range(k - 1, i, -1)), 1)
        for i in range(k - 1)
    ]
...
def li_test(ranks, control=None):
    ...
    adj_p_values = [
        p_values[i] / (p_values[i] + 1 - p_values[-1]) for i in range(k - 1)
    ]
...
def holm_multitest(ranks):
    ...
    m = int(k * (k - 1) / 2.0)
    adj_p_values = [
        min(max((m - j) * p_values[j] for j in range(i + 1)), 1) for i in range(m)
    ]
...
def hochberg_multitest(ranks):
    ...
    m = int(k * (k - 1) / 2.0)
    adj_p_values = [
        max((m + 1 - j) * p_values[j - 1] for j in range(m, i, -1)) for i in range(m)
    ]
...
def finner_multitest(ranks):
    ...
    m = int(k * (k - 1) / 2.0)
    adj_p_values = [
        min(max(1 - (1 - p_values[j]) ** (m / float(j + 1)) for j in range(i + 1)), 1)
        for i in range(m)
    ]
...
def shaffer_multitest(ranks):
    ...
    adj_p_values = [
        min(max(t[j] * p_values[j] for j in range(i + 1)), 1) for i in range(m)
    ]
